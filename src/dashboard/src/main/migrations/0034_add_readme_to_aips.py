...
def data_migration(apps, schema_editor):
    ...
    StandardTaskConfig.objects.create(
        ...
        execute='copy_v0.0',
        arguments='"%sharedPath%sharedMicroServiceTasksConfigs/README/README.html" "%SIPDirectory%README.html"'
    )
    ...
    MicroServiceChainLinkExitCode.objects.filter(microservicechainlink_id='ccf8ec5c-3a9a-404a-a7e7-8f567d3b36a0').update(nextmicroservicechainlink_id=add_readme_file_cl_pk)

    # Modify "Prepare AIP" chain link to include README.html in call to bagit_v0.0
    StandardTaskConfig.objects.filter(id='045f84de-2669-4dbc-a31b-43a4954d0481').update(arguments='create "%SIPDirectory%%SIPName%-%SIPUUID%" "%SIPDirectory%" "logs/" "objects/" "METS.%SIPUUID%.xml" "README.html" "thumbnails/" "metadata/" --writer filesystem')
